 ...
 * 虫情监控适配器
 */
public class PestMonAdapter extends Adapter<PestMonAdapter.MyViewHolder> {

    private Activity mActivity;

    public PestMonAdapter(Activity activity) {
        mActivity = activity;
    }
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        Glide.with(mActivity)
                .load(PestMonFragment.photoList.get(position)[0])
                .apply(new RequestOptions().placeholder(R.drawable.ambient_mr)
                        .error(R.drawable.ambient_mr))
                .into(holder.iv);
        holder.tv.setText(PestMonFragment.photoList.get(position)[1]);
    }

    @Override
    public int getItemCount() {
        return PestMonFragment.photoList.size();
    }
    ...
    class MyViewHolder extends ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            intent.putExtra("Position", new int[]{0, getAdapterPosition()});
